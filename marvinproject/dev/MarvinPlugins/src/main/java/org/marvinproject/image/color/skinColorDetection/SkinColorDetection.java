...
//		attributes = getAttributes();
	 ...
	 * @param imageIn,imageOut - the image to be rotated
	 * @param previewMode - to display a preview
	 */
	public void process
	(
		MarvinImage imageIn, 
	...
	)
	{
		...
		ColorSpaceConverter l_colorspaceConverter = MarvinPluginFactory.get(ColorSpaceConverter.class);
		l_colorspaceConverter.process(imageIn, l_hsvImage, attributesIn, attributesOut, mask, previewMode);
		
		
		/*The first step is to determine regions of the scene which
		 * have appropriate skin tone.
		 */
		findSkinColorPixels(imageIn, l_hsvImage, imageOut, mask);
	 ...
	 * @param imageOut
	 */
	private void findSkinColorPixels(MarvinImage imageIn, MarvinImage hsvImage, MarvinImage imageOut, MarvinImageMask mask)
	{
		...
		for(int xx = 0; xx < l_imageWidth; xx++)
		{
			...
			{
				if(l_arrMask != null && !l_arrMask[xx][yy]){
					continue;
				}
 ...
 * @param vector1,vector2 - the vectors who's angle of difference needs to be calculated
 */
